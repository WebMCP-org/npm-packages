...
/**
 * Server transport for Native Messaging using Port-based messaging.
 * This transport handles a single client connection through Chrome's native messaging Port API.
 * It should be used in the extension's background service worker.
 */
export class NativeServerTransport implements Transport {
  ...
  async start(): Promise<void> {
    if (this._started) {
      throw new Error(
        'NativeServerTransport already started! If using Server class, note that connect() calls start() automatically.'
      );
    }
