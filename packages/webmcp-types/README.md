...
MCP-B extensions and browser compatibility types built on the Community Group’s [`webmcp-types`](https://github.com/webmachinelearning/webmcp-types). This package emits declarations only and has no runtime side effects.
...
Activation loads upstream `WebMCP.ModelContext` for `document.modelContext` and adds optional deprecated navigator and declarative-form compatibility surfaces. Both type packages can be loaded together.
...
| Export                       | Purpose                                                    |
| ---------------------------- | ---------------------------------------------------------- |
| `ModelContextTool`           | Generic tool dictionary with legacy callback compatibility |
