...
type RegisteredTool = Pick<
  ...
  'description' | 'execute' | 'inputSchema' | 'name'
>;
type RegisteredToolConfig = Partial<Pick<ExtensionToolDescriptor, 'description' | 'inputSchema'>>;
...
describe('direct extension action tools', () => {
  ...
  it('registers the exact action-specific tool surface for the first-party Chrome API groups', () => {
    ...
    expect(server.tools.map((tool) => tool.name).sort()).toEqual([...DIRECT_TOOL_NAMES].sort());
