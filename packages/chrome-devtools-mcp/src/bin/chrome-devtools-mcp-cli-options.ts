...
export function parseArguments(version: string, argv = process.argv) {
  const yargsInstance = yargs(hideBin(argv))
    ...
    .example([
      ...
      ['$0 --no-performance-crux', 'Disable CrUX (field data) integration in performance tools.'],
      ['$0 --slim', 'Only 3 tools: navigation, JavaScript execution and screenshot'],
    ]);
  ...
  if (args.categoryExtensions === undefined) {
    args.categoryExtensions = false;
    args['category-extensions'] = false;
  }
