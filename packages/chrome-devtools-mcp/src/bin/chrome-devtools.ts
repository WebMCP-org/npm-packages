...
function parseObjectOption(argName: string, value: unknown) {
  ...
  try {
    ...
    throw new Error(`--${argName} must be a JSON object: ${message}`);
  }
}
      ...
      try {
        ...
        for (const argName of Object.keys(args)) {
          if (argName in argv) {
            ...
            commandArgs[argName] =
              arg.type === 'object' ? parseObjectOption(argName, value) : value;
          }
        }
