...
function normalizeCommandArgValue(
  commandName: string,
  argName: string,
  argType: string,
  value: unknown,
): unknown {
  if (argType !== 'object' || value === undefined) {
    return value;
  }
  ...
  if (typeof value !== 'string') {
    throw new Error(
      `Invalid value for ${commandName} --${argName}. Expected a JSON object string.`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON for ${commandName} --${argName}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }

  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(
      `Invalid value for ${commandName} --${argName}. Expected a JSON object.`,
    );
  }
    ...
    async argv => {
      try {
        ...
        for (const [argName, argDef] of Object.entries(args)) {
          if (argName in argv) {
            commandArgs[argName] = normalizeCommandArgValue(
              commandName,
              argName,
              argDef.type,
              argv[argName],
