...
function main() {
  ...
  const bundlePath = path.join(LIGHTHOUSE_DIR, 'dist', 'lighthouse-devtools-mcp-bundle.js');

  console.log(`Copying bundle from ${bundlePath} to ${DEST_DIR}...`);
  fs.copyFileSync(bundlePath, path.join(DEST_DIR, 'lighthouse-devtools-mcp-bundle.js'));
  ...
  fs.copyFileSync(noticesPath, path.join(DEST_DIR, 'LIGHTHOUSE_MCP_BUNDLE_THIRD_PARTY_NOTICES'));
