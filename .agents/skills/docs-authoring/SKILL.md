...
- When documenting `@mcp-b/*` packages, that's ours. Document fully. But clearly mark what is "WebMCP standard" behavior vs. "MCP-B extension" behavior. Treat `registerTool`, `getTools`, and `ontoolchange` as the current standard surface; registration lifetime belongs to `AbortSignal`. Do not describe `unregisterTool`, `provideContext`, or `clearContext` as current WebMCP standard methods. Everything else (`registerPrompt`, `registerResource`, `listTools`, `callTool`, `createMessage`, `elicitInput`) is an MCP-B extension.
