...
export class AlarmsApiTools extends BaseApiTools {
		...
		try {
			...
			if (!chrome.alarms) {
				return {
					...
					details:
						'This extension needs the "alarms" permission in its manifest.json',
				};
			}
			...
			if (typeof chrome.alarms.getAll !== "function") {
				return {
					...
					details:
						"The alarms API appears to be partially available. Check manifest permissions.",
				};
			}
	...
	private registerCreateAlarm(): void {
		this.server.registerTool(
			...
			{
				description:
					"Create an alarm that fires at a specific time or periodically",
				inputSchema: {
					name: z
						.string()
						.optional()
						.describe("Optional name to identify this alarm"),
					delayInMinutes: z
						.number()
						.min(0.5)
						.optional()
			...
			async ({ name, delayInMinutes, periodInMinutes, when }) => {
				try {
					...
					const createdAlarm = await new Promise<
	...
	private registerGetAlarm(): void {
		this.server.registerTool(
			...
			async ({ name }) => {
				try {
					const alarm = await new Promise<chrome.alarms.Alarm | undefined>(
						(resolve, reject) => {
							if (name) {
								chrome.alarms.get(name, (alarm) => {
									if (chrome.runtime.lastError) {
	...
	private registerGetAllAlarms(): void {
		this.server.registerTool(
			...
			async () => {
				try {
					const alarms = await new Promise<chrome.alarms.Alarm[]>(
						(resolve, reject) => {
					...
					return this.formatJson({
						...
						alarms: alarms.map((alarm) => ({
							name: alarm.name,
							scheduledTime: alarm.scheduledTime,
							scheduledTimeFormatted: new Date(
								alarm.scheduledTime,
							).toISOString(),
							periodInMinutes: alarm.periodInMinutes,
						})),
		...
		this.server.registerTool(
			...
			async () => {
				try {
					...
					const alarmsBefore = await new Promise<chrome.alarms.Alarm[]>(
						(resolve, reject) => {
							chrome.alarms.getAll((alarms) => {
								if (chrome.runtime.lastError) {
									reject(new Error(chrome.runtime.lastError.message));
