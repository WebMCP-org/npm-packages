...
keywords:
  ['WebMCP', 'WebMCP API', 'WebMCP proposal', 'WebMCP vs MCP', 'MCPB', 'document.modelContext', 'AI agents', 'browser tools']
...
| | WebMCP | MCP |
|---|---|---|
| Execution environment | The web page and browser | A local process or remote service |
| Existing context | Page state, browser session, and visible interface | State explicitly exposed by the MCP server |
| Capability lifecycle | Changes as the page and user workflow change | Managed by the connected server |
| Primary fit | Actions that belong to a website | Tools, resources, and prompts outside the page |
