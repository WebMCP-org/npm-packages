...
removed Chromium preview API retained only by MCP-B compatibility shims.
Adapter boundaries reduce migration cost.
...
primary surface, using `getTools()` for discovery, feature-detecting Chrome's
optional `executeTool()` extension, using `AbortSignal` for cleanup, and
keeping legacy MCP-B helpers as compatibility APIs.
