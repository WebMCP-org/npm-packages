...
```json
{
  "mcpServers": {
    "chrome-devtools": {
      "command": "npx",
      "args": ["-y", "chrome-devtools-mcp@latest", "--category-experimental-webmcp"]
    }
  }
}
```

The upstream WebMCP category currently requires Chrome 149 or newer. When the server launches Chrome, it enables the required WebMCP testing and DevTools features. The server launches Chrome when the agent first uses a browser tool; connecting to the MCP server alone does not start it.
...
When a webpage has `@mcp-b/global` installed, the server detects its tools through the Chrome DevTools Protocol. The upstream server exposes two WebMCP tools:
...
If you attach to an existing Chrome session, launch that browser with `--enable-features=WebMCPTesting,DevToolsWebMCPSupport` first.
...
| Problem                           | Fix                                                                                                                                      |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| WebMCP tools are unavailable      | Add `--category-experimental-webmcp` and use Chrome 149 or newer                                                                         |
