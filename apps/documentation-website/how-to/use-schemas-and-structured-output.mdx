...
When `outputSchema` is a literal schema and you use `@mcp-b/webmcp-types`, `structuredContent` is type-checked against the schema at compile time. WebMCP execution accepts any JSON value that matches the schema. This follows MCP [SEP-2106](https://modelcontextprotocol.io/seps/2106-json-schema-2020-12), which aligns tool output schemas and structured content with JSON Schema 2020-12.
...
<Warning>
  ...
  matches the schema. The MCP transport bridge currently forwards object-shaped `structuredContent`;
  use the WebMCP `executeTool()` path for primitive or array structured outputs until all downstream
  MCP clients accept SEP-2106 shapes.
</Warning>
