...
4. **Do you need MCP bridge transport, `listTools`, prompts, resources, elicitation, or sampling?** Install `@mcp-b/global`.
...
| `@mcp-b/global`          | Yes     | `registerTool`, `getTools` | `listTools`, prompts, resources, elicitation, sampling | Tab + iframe        | No    |
...
`@mcp-b/global` auto-initializes on import. It installs the polyfill first, then replaces `document.modelContext` with a `BrowserMcpServer` that keeps the core behavior and adds MCP-B-only methods such as `listTools`, prompts, resources, and transport support.
