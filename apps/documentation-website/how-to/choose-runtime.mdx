...
`@mcp-b/global` auto-initializes on import. It installs the polyfill first, then replaces `document.modelContext` with a `BrowserMcpServer` that keeps the core behavior and adds `listTools`, prompt and resource registration, the composed `mcpServer`, and transport support. Application code still uses `document.modelContext`. For manual initialization, set `autoInitialize: false` before the package loads, then import and call `initializeWebModelContext()` as shown in the [global reference](/packages/global/reference#configuration).
