...
The standalone polyfill follows the upstream object-input `executeTool()` API. `@mcp-b/global`
accepts object input and keeps a serialized-JSON compatibility overload; set
`nativeExecuteToolInput: 'json'` when wrapping an older native implementation.
