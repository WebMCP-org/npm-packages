...
Tools registered after `embed.js` loads are discovered through WebMCP `toolchange` events across the
iframe bridge; script order does not control discovery.
...
| `No tools listed`      | Confirm the page registered tools and WebMCP is available in the host and widget frames        |
| `Tool not found`       | The tab reloaded or disconnected. Call `webmcp_list_tools` to refresh                          |
| Connection blocked     | Verify `--widget-origin` matches the WebSocket origin and relay port matches `data-relay-port` |
