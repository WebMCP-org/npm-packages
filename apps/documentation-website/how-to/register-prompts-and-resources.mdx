...
<Steps>
  <Step title="Register the prompt">
    ```typescript
    const { unregister } = server.registerPrompt({
      name: 'greeting',
...
<Steps>
  <Step title="Register the resource">
    ```typescript
    const { unregister } = server.registerResource({
      uri: 'app://config',
