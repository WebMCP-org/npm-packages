...
The embed script injects a hidden iframe that opens a WebSocket to the relay on `localhost`. Page code registers tools on `document.modelContext`. The embed supports current native Chrome discovery through asynchronous `getTools()` and executes the exact returned descriptor through Chrome's feature-detected `executeTool()` extension.
...
| `No tools listed`        | Ensure tools are registered on the page's WebMCP runtime. If tools register after load, confirm your runtime emits `toolchange`; older MCP-B shims may use `registerToolsChangedCallback`. |
