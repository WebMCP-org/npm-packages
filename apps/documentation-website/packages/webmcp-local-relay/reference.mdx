---
title: '@mcp-b/webmcp-local-relay reference'
...
after the embed loads. The widget listens for WebMCP `toolchange` events through the iframe bridge;
it does not poll for tool updates.
...
The embed script fetches the sibling `widget.html`, injects configuration, and loads it as a hidden blob iframe that inherits the host page origin. The widget bundles the [upstream WebMCP polyfill](https://github.com/webmachinelearning/webmcp-polyfill) and calls its installer. It discovers tools through asynchronous `getTools()` and executes the returned descriptors with object input. When native WebMCP is unavailable, the polyfill frame bridge carries discovery, execution, and tool changes between the widget and host page; native WebMCP handles those calls when available. `postMessage` carries only reconnect and reload controls. The widget opens a WebSocket to the relay on `localhost`. Self-hosted copies must serve both `embed.js` and `widget.html`; cross-origin hosts must allow the widget fetch with CORS. The relay fails closed if that fetch fails.
...
2. Native Chrome with cross-frame `getTools()` and object-input `executeTool()`
...
| `No tools listed`          | Ensure the page registered tools and WebMCP is available in the host and widget frames                                                                      |
...
| `Tool execution timed out` | The page tool exceeded the per-request timeout (default 60s); increase it with `data-request-timeout="<ms>"` on the embed script and CLI `--invoke-timeout` |
