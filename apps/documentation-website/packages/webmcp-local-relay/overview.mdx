...
- You need browser automation, traces, console access, or DOM inspection. Use upstream [`chrome-devtools-mcp`](https://github.com/ChromeDevTools/chrome-devtools-mcp).
- You only need in-browser discovery through a runtime or extension.

## Where it sits in the package graph

This package is adjacent to the runtime stack rather than inside it. Pages need a WebMCP runtime,
usually [`@mcp-b/global`](/packages/global/overview), to expose tools on `document.modelContext`. They
load `embed.js` separately to bridge those existing tools to the relay.
