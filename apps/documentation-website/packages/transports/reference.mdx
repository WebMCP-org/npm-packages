...
| Extension to extension  | `ExtensionServerTransport` (in host extension) | `ExtensionClientTransport` (in guest extension) |
...
Cross-origin communication between a parent page and an iframe. Uses `window.postMessage` with a ready handshake to handle iframe loading timing.
...
Connects to the extension's background service worker via `chrome.runtime.connect`. A port
disconnect closes the MCP connection. Create a new transport and call `client.connect()` again so
the new server session completes MCP initialization.
