...
keywords: ["@mcp-b/webmcp-polyfill", "polyfill", "document.modelContext", "initializeWebMCPPolyfill", "registerTool", "modelContextTesting", "strict core"]
icon: "puzzle-piece"
---
...
<Note>
The polyfill does not provide `listTools()` or `callTool()` on `document.modelContext`. For tool listing and execution, use `navigator.modelContextTesting` (when the testing shim is enabled) or switch to [`@mcp-b/global`](/packages/global/reference).
</Note>
