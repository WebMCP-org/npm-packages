...
| `outputSchema`             | JSON Schema                       | No       | Defines output structure and enables `structuredContent`.  |
...
When `outputSchema` is defined, the implementation must return a JSON-serializable value that matches that schema. Object, array, string, number, boolean, and null schemas are supported for WebMCP execution.
