...
<Note>
  Native cross-document WebMCP does not require this package. It uses iframe `allow="tools"`,
  child-side `exposedTo`, and parent-side `getTools({fromOrigins})`. Those browser controls are
  independent of the MCP transport's `target-origin` and `allowedOrigins`.
</Note>
