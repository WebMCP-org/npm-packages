...
- You only need native strict-tool discovery across a document tree. Use the
  browser's `allow="tools"`, `exposedTo`, and `fromOrigins` surfaces instead.
...
| Child setup   | `allow="tools"` delegation and `exposedTo` | MCP server connected to `IframeChildTransport`      |
