...
`useWebMCP(config, deps?)` returns `execute`, `isSupported`, and `registrationError`.
It does not allocate or subscribe to execution state. Attach an `executionState()` plugin and
call `useToolExecutionState(execution)` to display status. The
[shared React API](/packages/usewebmcp/reference#hooks) includes a complete example.

The hook accepts named `plugins`, `binding`, and `checkBinding`. Standard Schema validation
runs before consent. Managed MCP input/output validation and response classification stay inside
the same invocation. An attached execution store records MCP error responses as failures;
local calls retain raw objects, including those with `isError`.

The [plugin package](https://github.com/WebMCP-org/npm-packages/tree/main/packages/webmcp-plugins)
owns consent, state, tracing, and the framework-independent runner.
...
validation, cancellation, `enabled`, `exposedTo`, and registration errors come from the core hook.
...
**Returns:** `WebMCPReturn`, containing `execute`, `isSupported`, and `registrationError`.
Tool hooks no longer return `isRegistered`; use the runtime’s `getTools()` for confirmed discovery.

Agent failures default to `{ content: [{ type: "text", text: error.message }], isError: true }`.
`formatError` overrides this response. Both formatters may be async and are awaited. Local failures
and cancellation always reject. An optional execution store retains failures separately
from `registrationError`. See [response behavior](/packages/usewebmcp/reference#tool-responses).
...
The handler's return type is inferred for local execution and plugins, even without an
`outputSchema`. When supplied, the output schema constrains that result type.

`enabled` controls registration, not local execution. See
[registration configuration](/packages/usewebmcp/reference#config-fields) for registration and
...
| `options`     | `{ enabled?: boolean; plugins?: WebMCPPlugin[] }` | Optional registration control; `enabled` defaults to `true` |
...
| `ToolExecutionState`                     | Optional execution observer snapshot                             |
| `WebMCPConfig<TInput, TOutput, TResult>` | Hook configuration; inferred result constrained by output schema |
| `WebMCPReturn<TOutput, TInput, TResult>` | Registration controls and typed local execution                  |
...
`useWebMCPTool` and the tool hook's implicit `state`/`reset` return are removed.
Use `useWebMCP`, named `plugins`, and an explicit execution store/subscription.
Plugin imports belong to `@mcp-b/webmcp-plugins`; the old polyfill entries are removed.
`inputSchema: vendorSchema` remains supported. Prompt/resource registration status is unchanged.
