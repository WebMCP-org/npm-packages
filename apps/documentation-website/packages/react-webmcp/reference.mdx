...
Registers an MCP prompt through the `BrowserMcpServer` installed by `@mcp-b/global`. It does not register against a native strict-only context.
...
Registers an MCP resource through the `BrowserMcpServer` installed by `@mcp-b/global`. It does not register against a native strict-only context.
