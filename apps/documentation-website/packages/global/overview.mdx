...
`@mcp-b/global` is the full runtime entry point for browser applications. It installs or adopts the strict WebMCP surface, creates a `BrowserMcpServer`, connects transport, and returns the active server for typed extension access.
...
Import the package as early as possible in the browser entry point, then register tools on `document.modelContext`. Call `initializeWebModelContext()` and keep its return value when you need MCP-B extensions. Use the [reference page](/packages/global/reference) when you need configuration, cleanup, or transport details.
