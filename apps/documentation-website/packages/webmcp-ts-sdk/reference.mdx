...
`BrowserMcpServer` normalizes JSON Schema and Standard JSON Schema inputs through the
[polyfill's optional schema adapter](/packages/webmcp-polyfill/reference#schema-entry). If the schema
also supplies `~standard.validate()`, the adapter preserves it for the composed `McpServer`.
...
[`usewebmcp`](/packages/usewebmcp/reference) already applies it for React registrations.
