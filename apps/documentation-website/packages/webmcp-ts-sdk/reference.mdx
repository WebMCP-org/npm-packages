---
title: '@mcp-b/webmcp-ts-sdk reference'
...
Executes a descriptor returned by `getTools()`. Object input follows the current WebMCP draft; serialized JSON remains accepted as an MCP-B compatibility overload. The optional signal aborts the call. Native execution uses object input by default; set `nativeExecuteToolInput: 'json'` when wrapping an older native implementation.
...
Reconciles the adapter with tools already visible through `native.getTools()`. Backfill uses the native context's descriptor-based `executeTool()` when available, with object input by default or serialized JSON when `nativeExecuteToolInput: 'json'` is configured. Later native `toolchange` events reconcile additions, metadata changes, and removals.
...
`BrowserMcpServer` normalizes metadata with the
[polyfill schema helper](/packages/webmcp-polyfill/reference#schema-entry).
For unwrapped callbacks, the official MCP server runs a preserved Standard Schema validator
or its JSON Schema adapter on MCP calls. Direct browser execution bypasses that validation.

Callbacks created by
[`@mcp-b/webmcp-plugins`](https://github.com/WebMCP-org/npm-packages/tree/main/packages/webmcp-plugins#register-a-callback-outside-react)
are explicitly managed. Their JSON metadata is projected separately, and MCP input/output
validation runs inside the shared invocation, where tracing and state observe failures.
The vendor input transform runs once before consent and execution.
[`usewebmcp`](/packages/usewebmcp/reference) creates managed callbacks automatically.
The [schema guide](/how-to/use-schemas-and-structured-output) shows direct registration.

`outputSchema` is enforced on MCP calls. Direct browser and local calls do not perform that check.
MCP error classification applies to valid protocol responses, not arbitrary raw objects.
Trusted protocol/trace metadata is passed separately from tool arguments.

MCP requires object-root tool input schemas. Array-root WebMCP tools remain available through
WebMCP and are omitted from MCP discovery with a warning. `PromptDescriptor.argsSchema` accepts
`InputSchema`. Direct official SDK registrations support Zod 4.2 or newer; Zod 3 is unsupported.
