...
Executes a descriptor returned by `getTools()` using the package's serialized-JSON compatibility signature. The optional signal aborts the call. The [live proposal](https://webmachinelearning.github.io/webmcp/) currently specifies object input, while the [Chrome imperative API](https://developer.chrome.com/docs/ai/webmcp/imperative-api) documents serialized JSON.
...
Reconciles the adapter with tools already visible through `native.getTools()`. Backfill requires the native context's descriptor-based, serialized-JSON `executeTool()` signature. Later native `toolchange` events reconcile additions, metadata changes, and removals.
