...
WebMCP uses [Changesets](https://github.com/changesets/changesets) for versioning. The browser packages share one fixed release version. Changesets bumps the full group when any member changes. `@mcp-b/smart-dom-reader-server` is versioned independently. Private apps, templates, and test packages are not published.
...
`@mcp-b/smart-dom-reader-server` follows its own `0.x` version line. See the
[package index](/packages/index) for package-specific documentation.
