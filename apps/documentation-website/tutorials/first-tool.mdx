...
You have registered a WebMCP tool, discovered it through `getTools()`, and executed it through the upstream object-input API.
...
- `document.modelContext.getTools()` discovers registered tools; the upstream polyfill accepts an object as `executeTool()` input
