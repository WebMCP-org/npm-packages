...
You should see an array containing your `say_hello` tool. Now feature-detect the upstream object-input execution method and call it:
...
const result = await executeTool.call(modelContext, tool, { name: 'Console' });
...
- `useWebMCP` returns local `execute`; the `executionState()` plugin records calls and `useToolExecutionState()` subscribes the UI
- Tools registered via the hook are discoverable with `document.modelContext.getTools()` and callable when the runtime exposes object-input `executeTool()`
