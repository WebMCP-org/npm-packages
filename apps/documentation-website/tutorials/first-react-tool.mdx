...
You should see an array containing your `say_hello` tool. Now feature-detect the Chrome-compatible execution extension and call it:
...
- Tools registered via the hook are discoverable with `document.modelContext.getTools()` and callable when the runtime exposes Chrome's optional `executeTool()` extension
