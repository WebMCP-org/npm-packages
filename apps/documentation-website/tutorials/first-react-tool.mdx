...
- Tools registered via the hook are discoverable with `document.modelContext.getTools()` and callable when the runtime exposes its object-input `executeTool()` method
