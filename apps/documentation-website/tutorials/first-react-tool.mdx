...
```bash title="Create the React app"
...
```bash title="Install the hook and polyfill"
pnpm add @mcp-b/webmcp-polyfill @mcp-b/webmcp-plugins usewebmcp
```
...
export function App() {
  const [execution] = useState(() => executionState<{ message: string }>());
  ...
  return (
    <div>
      ...
      <p>Last result: {state.lastResult ? JSON.stringify(state.lastResult) : 'none'}</p>
...
```javascript title="Find the registered tool"
...
```javascript title="Call the tool through the polyfill"
...
```json title="Expected agent result"
{
...
- The hook returns `execute` for local calls; the `executionState()` plugin records calls and `useToolExecutionState()` subscribes the UI
...
- See the [`usewebmcp` reference](/packages/usewebmcp/reference) for the full hook API, Standard Schema validation, and cancellation
