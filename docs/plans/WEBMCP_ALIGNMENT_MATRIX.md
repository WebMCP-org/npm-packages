...
| ------ | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| WMC-01 | Removed context APIs                | `provideContext()` and `clearContext()` are no longer exposed.                                                                                          | Native preview does not expose `provideContext()` or `clearContext()`.                                                   | Resolved parity item                       | P0       | Keep these APIs absent from strict WebMCP core types, polyfill runtime, and bridge runtime.                                                           |
...
| WMC-13 | Async `registerTool()` hardening    | Public types stay spec-aligned (`void`), while SDK native mirroring, React registration, and native-showcase compatibility observe thenable rejections. | Chrome Canary 150 returns `undefined` from `registerTool`; the May 27, 2026 draft also defines `undefined registerTool(...)`. | Forward-compat risk                        | P1       | Keep public API sync unless upstream IDL changes; observe any future thenable result to avoid unhandled rejections and preserve abort-signal cleanup. |
...
2. Keep tests that assert removed context APIs stay absent from strict native and polyfill surfaces.
